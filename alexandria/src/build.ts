...
export const build = async (config: BuildConfig) => {
	const { outDir, layouts, articles, componentsDir, template, publicDir } = config;
...
async function processComponents(componentsSourceDir: string, componentsOutDir: string) {
	...
	for (let i = 0; i < componentPaths.length; i++) {
		...
		const transformedCode = transform(componentSource, {
			...babelConfig,
		...
		await asyncWriteFile(componentOutPath, transformedCode!.code!);
	}
...
async function processMdxArticle(articlePath: string, articleContents: string): Promise<ArticleResult> {
	...
	const hash = createHash("sha256").update(articleContents).digest("hex");
...
async function writeApplication(config: WriteApplicationConfig) {
	...
	for (let i = 0; i < layoutResults.length; i++) {
		...
		const transformedLayoutCode = transform(layoutResult.jsxContents, {
			...babelConfig,
			filename: layoutResult.articlePath,
		});

		await asyncWriteFile(join(outDir, "layouts", `${layoutResult.hash}.js`), transformedLayoutCode!.code!);
	}
	...
	for (let i = 0; i < articleResults.length; i++) {
		...
		const transformedArticleCode = transform(articleResult.jsxContents, {
			...babelConfig,
			filename: articleResult.articlePath,
		});

		await asyncWriteFile(join(outDir, "articles", `${articleResult.hash}.js`), transformedArticleCode!.code!);
	...
	const usedComponents = Array.from(
		Object.keys(allDynamics).reduce((usedComponents, dynamicsName) => {
...
ReactDOM.render(
	...
	const aliases = usedComponents.reduce<any>((aliases, componentName) => {
